#Script to calculate indexes for current period. This script is run without using ebird data. 
# load libraries
library(dplyr)
library(sf)
library(mapview)
...
FilteredCP <- read.csv("Data/FilteredCP_sensitivity.csv", stringsAsFactors = F)
...
PAs2022 <- st_read("Spatial_Data/Protected areas_enp_2022/Enp2022_p.shp")
...
write_sf(Indexes_pcells2_PA_ZPAS, "Spatial_Data/combindexCP_26June_sensitivity.shp")#This shp contains the combined index for all 5070 cells.
...
combindexHP <- st_read("Spatial_Data/combindexHP_26June_senitivity.shp")
combindexCP <- st_read("Spatial_Data/combindexCP_26June_sensitivity.shp")
...
ggsave("Figures/combindsHPCP_26June_sensitivity.png", combined_plot, wi = 50, he = 40, un = "cm", dpi = 300)
...
ggsave("Figures/hotspotschange26june_sensitivity.png", map_change, wi = 20, he = 20, un = "cm", dpi = 300)
write_sf(combindexHP_CP_1, "Spatial_Data/combindexHP_CP_26June_sensitivity.shp")#This shp contains the combined index only for cells with data for both periods. 
...
#Caculating correlation between dataset with ebird data vs dataset without ebird data
combindex_HP_CP_webird <- st_read("Spatial_Data/combindexHP_CP_26June.shp")
combindex_HP_CP_sensitivity <- combindexHP_CP_1

names(combindex_ebird)

combindex_HP_CP_webird <- combindex_HP_CP_webird[, c(1,12,26)]
combindex_HP_CP_sensitivity <- combindex_HP_CP_sensitivity[, c(1,12,26)]

combindex_HP_CP_webird$geometry <- NULL
combindex_HP_CP_sensitivity$geometry <- NULL

df_merged <- merge(combindex_HP_CP_webird, combindex_HP_CP_sensitivity, by = "UTMCODE")
correlacion1 <- cor(df_merged$cmb_nHP.x, df_merged$cmb_nHP.y, use = "complete.obs")
correlacion2 <- cor(df_merged$cmb_nCP.x, df_merged$cmb_nCP.y, use = "complete.obs")

# Now calculating correlations among indices from the whole dataset that used ebird data, as requested by the reviewer
combindex_ebird <- st_read("Spatial_Data/combindexHP_CP_26June.shp")

corrmatrix <- combindex_ebird[, c(2:6)]
corrmatrix$geometry <- NULL
corrmatrix  <- as.data.frame(cor(corrmatrix))

cor(corrmatrix2)

corrmatrix2 <- combindex_ebird[, c(16:20)]
corrmatrix2$geometry <- NULL
corrmatrix2  <- as.data.frame(cor(corrmatrix2))
write_xlsx(corrmatrix2, 'Data/CIa_indices_correlation.xlsx') 
