#Script to calculate indexes for historical and current period. This script is run without using ebird data. 
  ...
  # species per cell
  Spp_percellCP <- atlas16 %>% 
    group_by(UTMCODE) %>% 
    summarize(Species = n())
  
  names (Spp_percellCP)[2] = "N_Species_CP"
  
  #Identifying cells with data for both periods. 4231 out of 4231 cells have data for both periods.
  ...
  FilteredCP <- merge(atlas16, Spp_percellHPCP, by="UTMCODE")
  ...
  # Percentage of cells occupied by each species from the total 4231 cells
  Species_cellHP$Perc_cellsHP  <- (Species_cellHP$N_cellsHP*1/4231)
...
#This shp contains the combined index for all 4231 cells in HP.
write_sf(Indx_PASPA_HP, "Spatial_Data/combindexHP_26June_senitivity.shp") 
...
write.csv(FilteredHP, "Data/FilteredHP_sensitivity.csv", row.names = FALSE)
write.csv(FilteredCP, "Data/FilteredCP_sensitivity.csv", row.names = FALSE) #This one will be used as well in the next script for CP analysis.
